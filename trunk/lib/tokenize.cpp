...
fstream tokenlogfile("tokenize_time.log",ios::out);
Tokenizer::Tokenizer() :
    list(0),
    _settings(0),
    _errorLogger(0),
...
void Tokenizer::duplicateDeclarationError(const Token *tok1, const Token *tok2, const std::string &type) const
{
    ...
    //reportError(locationList, Severity::style, "unnecessaryForwardDeclaration","unnecessaryForwardDeclaration",
...
void Tokenizer::unsupportedTypedef(const Token *tok) const
{
	...
	;
#else
    reportError(tok1, Severity::debug, "debug", "debug",
                         ...
                         const bool preprocessorCondition)
{
	...
	,filename+" createTokens ",tokenlogfile)
    // if MACRO
	TIMELOG(
    for (Token *tok = list.front(); tok; tok = tok->next()) {
    ...
    for (Token *tok = list.front(); tok && tok->next(); tok = tok->next()) {
		...
		//combine "private"and":" to "private:" 
    ...
    // enum..
	// TSC:from TSC 20140227 xiaolv
    //simplifyEnum();

    // Remove __asm..
    simplifyAsm();
	, " simplifyAsm ",tokenlogfile)
    ...
    simplifyStructInit();
	,"simplifyStructInit ",tokenlogfile)
	...
	,"simplifyVariableMultipleAssign ",tokenlogfile)
   
	TIMELOG(
    // Remove redundant parentheses
    simplifyRedundantParenthesis();
	, "simplifyRedundantParenthesis ",tokenlogfile)

	TIMELOG(
    for (Token *tok = list.front(); tok; tok = tok->next())
	{
		//std::cout<<FileName<<"tokenizer simplify  end  144 while start"<<tok->str()<<endl;
        while (TemplateSimplifier::simplifyNumericCalculations(tok));
		//std::cout<<FileName<<"tokenizer simplify  end  144 while end"<<endl;
	}
	...
	,"simplifyTemplates ",tokenlogfile)

	TIMELOG(
    // Simplify the operator "?:"
    simplifyConditionOperator();
	,"simplifyConditionOperator ",tokenlogfile)
	...
	,"simplifyPointerToStandardType ",tokenlogfile)

	TIMELOG(
    // simplify function pointers
    simplifyFunctionPointers();
	, "simplifyFunctionPointers ",tokenlogfile)
	...
	, "simplifyLogicalOperators ",tokenlogfile)

	TIMELOG(
	//from TSC 20141113
	simplifyVarParenthesis();
	,"simplifyVarParenthesis ",tokenlogfile)

	TIMELOG(
	//from TSC 20140902
	simplifyNOT();
	,"simplifyNOT ",tokenlogfile)

    // Change initialisation of variable to assignment
	TIMELOG(
    simplifyInitVar();
	, " simplifyInitVar ",tokenlogfile)

	// Split up variable declarations.
	TIMELOG(
    simplifyVarDecl(false);
	, " simplifyVarDecl ",tokenlogfile)
	...
	, " createLinks2&simplifyInitVar ",tokenlogfile)

	TIMELOG(
    simplifyMathFunctions();
	, "simplifyMathFunctions ",tokenlogfile)

	TIMELOG(
    simplifyDoublePlusAndDoubleMinus();
	, "simplifyDoublePlusAndDoubleMinus ",tokenlogfile)

	TIMELOG(
    simplifyArrayAccessSyntax();
    list.front()->assignProgressValues();
    removeRedundantSemicolons();
	, "simplifyArrayAccessSyntax ",tokenlogfile)

	TIMELOG(
    simplifyParameterVoid();
	, "simplifyParameterVoid ",tokenlogfile)
	
	TIMELOG(
    simplifyRedundantConsecutiveBraces();
	, "simplifyRedundantConsecutiveBraces ",tokenlogfile)
	
	TIMELOG(
	simplifyEmptyNamespaces();
	, "simplifyEmptyNamespaces ",tokenlogfile);

    bool valid = validate();
	TIMELOG(
    if (valid)
        createSymbolDatabase();
	, "createSymbolDatabase ",tokenlogfile)
...
void Tokenizer::simplifyNull()
{
    for (Token *tok = list.front(); tok; tok = tok->next()) {
        if (tok->str() == "NULL" || tok->str() == "__null" ||
			...
			if(tok->str() == "NULL" || tok->str() == "__null") // TSC:from TSC 20140126 add isNull
			{
				tok->isNull(true);
			}
...
void Tokenizer::simplifyNOT()
{
	...
	for (Token *tok = list.front(); tok; tok = tok->next()) {
		if(Token::Match(tok,"if|while ( ! (")){
				...
				bool flag=false;
				bool isNeedSimplify=true;
				//if(!(a>b && c>d)?1:0)) is not need simplify
				if(tok->tokAt(4) && tok->tokAt(4)->str()=="(")
				{
					continue;
				}
		        //if(!(a<=0|| a>=8)) is not need simplify
				Token* tmp=tok->tokAt(3);
				while(end && tmp && tmp!=end)
				{
					if(tmp->type()==Token::eComparisonOp ||tmp->type()==Token::eArithmeticalOp )
					{
						
						isNeedSimplify=false;
						break;
					}
					tmp=tmp->next();
				}
				...
				if(start)
				tok=start->tokAt(2);
				while(isNeedSimplify && end && tok && tok!=end)
				{
					//if(!( a && (b ||c))=>if(!a || !(b||C))
					if(Token::Match(tok,"("))
					{
...
static bool setVarIdParseDeclaration(const Token **tok, const std::map<std::string,unsigned int> &variableId, bool executableScope)
{
	...
	//from TSC 201402813
	/*ignore this case: "typedef class/union/struct TString{...}TString;"
	 *after tokenized: "typedef class/union/struct TString{...}; TString TString;"
	 *filt this expression"TString TString;" as a variable declaration
	 */
	bool flag_typedef=false;

	if(typeCount >= 2 && tok2 && Token::Match(tok2->tokAt(-2), "!!:: %type%"))
	{
		const Token *tok_end = tok2->tokAt(-4);
		const Token *tok_start = NULL ;
		const Token *tok_1 = NULL ;
		const Token *tok_2 = NULL ;
		if(tok_end&&tok_end->str() == "}")
		{
			tok_start = tok_end->link();
			if(tok_start)
			{
				tok_1 = tok_start->tokAt(-1);
				tok_2 = tok_start->tokAt(-2);
			}

			if (tok_1&&(tok_1->str() == "class" || tok_1->str() == "struct" || tok_1->str() == "union")&&tok_1->previous()&&(tok_1->previous()->str()=="typedef"))
				flag_typedef=true;
			else
				if (tok_2&&(tok_2->str() == "class" || tok_2->str() == "struct" || tok_2->str() == "union")&&tok_2->previous()&&(tok_2->previous()->str()=="typedef"))
					flag_typedef=true;
		}
	}
	return ((typeCount >= 2 && tok2 && Token::Match(tok2->tokAt(-2), "!!:: %type%"))&&!flag_typedef);
}
...
{
    Token *tok = *tok1;
    while (tok && Token::Match(tok->next(), ". %var% !!(")) {
...
void Tokenizer::simplifyCasts()
{
    for (Token *tok = list.front(); tok; tok = tok->next()) {
		...
		// modified by TSC, 20141114
		// handle "use(123, (long&)nVal);"
...
// TSC:from 20140224
void Tokenizer::simplifyVarArray()
{
	for (Token *tok = list.front(); tok; tok = tok->next()) {
        if (Token::Match(tok, "%var% [") ) {
			if(tok && tok->previous() && tok->previous()->str()!=";" && tok->previous()->str()!="{" && tok->previous()->str()!="(")
			{
				Token *tok1=tok->next();
				if(tok1 && tok1->str()=="[")
					tok1=tok->link();			
...
void Tokenizer::simplifyIfNotNull()
{
    for (Token *tok = list.front(); tok; tok = tok->next()) {
        ...
        if (Token::Match(tok, "(|&&|%oror%")) {
            //from TSC 20141112 bug fixed here should not use tok=tok->next() to change tok (use flag) eg.  if(!((p1!=0) &&(p2!=0))=> if(!((p1) && (p2!=0))
			tok = tok->next();
			bool flag=false;
            if (!tok)
                break;
			...
			if(!flag)
			{
			tok=tok->previous();//from TSC 20141112 if flag is false means should not use tok=tok->next() to change tok so here recover it
			}
    ...
    for (; tok3; tok3 = tok3->next()) {
        ...
        if (tok3->varId() == varid) {
            ...
            if (_settings->debugwarnings) {
                ...
                reportError(tok3, Severity::debug, "debug","debug",
        ...
        if (indentlevel == indentlevel3 && Token::Match(tok3->next(), "++|-- %varid%", varid) && MathLib::isInt(value) &&
            !Token::Match(tok3->tokAt(3), "[.[]")) {
				//from TSC 20141023 bug fixed dot_pos[3] = -1; wrongly to dot_pos[4] = -1;
			if((*tok2)->tokAt(2)->varId()==varid && varid!=0)
			{
...
/*
*Remove redundant Parenthesis in Var
*@author TSC
*@date 20141113
*/
void Tokenizer::simplifyVarParenthesis()
{
	 for (Token *tok = list.front(); tok; tok = tok->next()) {
		...
		// keep parentheses here: (size_t)*p;
        // keep parentheses here: Functor()(a ... )
        // keep parentheses here: ) ( var ) ;
	
        if (tok->next() && Token::Match(tok->next(), "( %var% ) ;|)|,|]|%op%") &&
            !tok->isName() &&
            tok->str() != ">" &&
            tok->str() != "]" &&
...
void Tokenizer::duplicateEnumError(const Token * tok1, const Token * tok2, const std::string & type) const
{
    ...
    //reportError(locationList, Severity::style, "variableHidingEnum","variableHidingEnum",
        ...
        if (Token::Match(tok, "class|struct|namespace") && tok->next() &&
            ...
            // Substitute enum values
            {
                ...
                for (Token *tok2 = tok1->next(); tok2; tok2 = tok2->next()) {
					...
					else if (inScope &&    // enum is in scope, from TSC 20140227 xiaolv youhua
...
void Tokenizer::syntaxError(const Token *tok) const
{
	...
	;
	#else
      reportError(tok, Severity::error, "syntaxError", "syntaxError","syntax error");
	#endif
}
...
void Tokenizer::syntaxError(const Token *tok, char c) const
{
	...
	;
	#else
     reportError(tok, Severity::error, "syntaxError","syntaxError",
...
void Tokenizer::cppcheckError(const Token *tok) const
{
	...
	;
	#else
    reportError(tok, Severity::error, "checkError","checkError",
...
{
    reportError(tok, Severity::portability, "unnecessaryQualification","unnecessaryQualification",
...
//add by TSC get filePath by fileindex 
const std::string& Tokenizer::getFilePathByIndex(int fileIndex) const
{
    if (list.getFiles().empty()) {
        static const std::string empty;
        return empty;
    }
    return list.getFiles()[fileIndex];
}
...
// from TSC 20130617
void Tokenizer::reportError(const Token* tok, const Severity::SeverityType severity, const std::string& id,  const std::string& subid, const std::string& msg, bool inconclusive) const
{
    const std::list<const Token*> callstack(1, tok);
    reportError(callstack, severity, id, subid,msg, inconclusive);
}
// from TSC 20130617
void Tokenizer::reportError(const std::list<const Token*>& callstack, Severity::SeverityType severity, const std::string& id, const std::string& subid,  const std::string& msg, bool inconclusive) const
{
    ErrorLogger::ErrorMessage errmsg(callstack, &list, severity, id,subid, msg, inconclusive);
    if (_errorLogger)
