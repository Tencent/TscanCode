...
bool CheckNullPointer::isPointerDeRef(const Token *tok, bool &unknown)
{
    ...
    //[ac setupLocationManager];
	if (!tok->previous() || (Token::Match(tok->previous(), "[;({]") && Token::simpleMatch(tok->next(), "=")))
	{
		return false;
	}
...
{
	reportError(tok, Severity::error, ErrorType::None, "nullPointer", "Null pointer dereference", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckNullPointer::nullPointerError(const Token *tok, const std::string &varname, bool inconclusive, bool defaultArg)
{
    if (defaultArg) {
        if (_settings->isEnabled("warning"))
            reportError(tok, Severity::warning, ErrorType::None, "nullPointerDefaultArg", "Possible null pointer dereference if the default parameter value is used: " + varname, 0U, inconclusive);
    } else
        reportError(tok, Severity::error, ErrorType::None, "nullPointer", "Possible null pointer dereference: " + varname, 0U, inconclusive);
}
...
void CheckNullPointer::nullPointerError(const Token *tok, const std::string &varname, const Token* nullCheck, bool inconclusive)
{
    ...
    reportError(callstack, Severity::warning, ErrorType::None, "nullPointerRedundantCheck", errmsg, 0U, inconclusive);
}
