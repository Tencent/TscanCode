...
{
	reportError(tok, Severity::warning, ErrorType::Suspicious, "autovar",
		"Address of an auto-variable returned.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckAutoVariables::errorReturnPointerToLocalArray(const Token *tok)
{
	...
	{
		reportError(tok, Severity::warning, ErrorType::Suspicious, "autovar", 
			"Pointer to local array variable returned.", ErrorLogger::GenWebIdentity(tok->str()));
	}
}
...
void CheckAutoVariables::errorAutoVariableAssignment(const Token *tok, bool inconclusive)
{
    if (!inconclusive) {
		reportError(tok, Severity::warning, ErrorType::Suspicious, "autovar",
					...
					"is invalid after the function ends.", ErrorLogger::GenWebIdentity(tok->str()));
    } else {
		reportError(tok, Severity::warning, ErrorType::Suspicious, "autovar",
                    "Address of local auto-variable assigned to a function parameter.\n"
...
{
	reportError(tok, Severity::warning, ErrorType::None, "autovar",
				...
				"value is invalid.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckAutoVariables::errorUselessAssignmentArg(const Token *tok)
{
	reportError(tok, 
				...
				"Assignment of function parameter has no effect outside the function.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckAutoVariables::errorUselessAssignmentPtrArg(const Token *tok)
{
	reportError(tok,
				...
				"Assignment of function parameter has no effect outside the function. Did you forget dereferencing it?", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckAutoVariables::errorReturnReference(const Token *tok)
{
	if (tok && !(tok->isExpandedMacro()))
	{
		reportError(tok, Severity::warning, ErrorType::Suspicious, "autovar", "Reference to auto variable returned.", ErrorLogger::GenWebIdentity(tok->str()));
	}
}

void CheckAutoVariables::errorReturnTempReference(const Token *tok)
{
	reportError(tok, Severity::warning, ErrorType::None, "returnTempReference", "Reference to temporary returned.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckAutoVariables::errorInvalidDeallocation(const Token *tok)
{
    reportError(tok,
		Severity::warning, ErrorType::None,
				...
				"that has been allocated dynamically.", ErrorLogger::GenWebIdentity(tok->str()));
}

#pragma region From TSC 1.0

static bool CheckVarIsString(const Variable* pVar)
{
	const Token* tokEnd = NULL;
	if (pVar->typeEndToken())
	{
		tokEnd = pVar->typeEndToken()->next();
	}

	for (const Token* tok = pVar->typeStartToken(); tok && tok != tokEnd; tok = tok->next())
	{
		if ((tok->str().find("string") != std::string::npos) || (tok->str().find("String") != std::string::npos))
		{
			return true;
		}
	}
...
void CheckAutoVariables::AssignNoneLocalVarWithLocalVarAddress()
{
	const SymbolDatabase *symbolDatabase = _tokenizer->getSymbolDatabase();
	...
	for (std::size_t i = 0; i < functions; ++i)
	{
		...
		for (const Token *tok = scope->classStart; tok && tok != scope->classEnd; tok = tok->next())
		{
			...
			if (scope->className == "main")
			{
				break;
			}
			// Critical assignment
			if (Token::Match(tok, "[;{}] %var% = %var% ;") &&
				isAutoVarArray(tok->tokAt(3)))
			{
				const Variable * var = symbolDatabase->getVariableFromVarId(tok->tokAt(1)->varId());

				if (var && !var->isLocal())
				{
					//false positive bug fixed.eg. fun(char** p){ char* localp; p=&localp;}
					if (var->isArgument())
					{
						const Token* nametok = var->nameToken();
						if (nametok && nametok->previous() && nametok->previous()->str() != "&")
						{
							tok = tok->tokAt(4);
							continue;
						}
					}
					// assigned to string is OK
					if (var->typeStartToken())
					{

						if (!CheckVarIsString(var))
						{
							errorAssignNoneLocalVarWithLocalVarAddress(tok->next());
						}
					}
					tok = tok->tokAt(4);
					continue;
				...
				isAutoVarArray(tok->tokAt(5)))
			{
				...
				if (var && !var->isLocal())
				{
					...
					// assigned to string is OK
					const Scope* pType = var->typeScope();
					if (pType)
					{
						const Variable* pVarSub = pType->getVariable(tok->tokAt(3)->str());
						if (pVarSub)
						{
							if (!CheckVarIsString(pVarSub))
							{
								errorAssignNoneLocalVarWithLocalVarAddress(tok->tokAt(3));
							}
						}
					}
					tok = tok->tokAt(6);
				...
				isAutoVar(tok->tokAt(4)))
			{
				...
				if (var && !var->isLocal())
				{
					...
					errorAssignNoneLocalVarWithLocalVarAddress(tok->next()->next());
				}
			}
			else if (Token::Match(tok, "[;{}] %var% . %var% = & %var% ;") &&
				isAutoVar(tok->tokAt(6)))
			{
				const Variable * var = symbolDatabase->getVariableFromVarId(tok->tokAt(1)->varId());
				if (var && !var->isLocal())
				{
					...
					errorAssignNoneLocalVarWithLocalVarAddress(tok->tokAt(4));
				}
			}
		}
	}
...
void CheckAutoVariables::errorAssignNoneLocalVarWithLocalVarAddress(const Token *tok)
{
	if (tok && !(tok->isExpandedMacro()) && tok->next() && tok->next()->next() && !tok->next()->next()->isExpandedMacro())
	{
		const std::string &str = tok->strAt(2);
		reportError(tok, Severity::warning, ErrorType::Suspicious,
			"autovar", "Returning the address of a stack variable [" + str + "].", ErrorLogger::GenWebIdentity(str));
	}
}
