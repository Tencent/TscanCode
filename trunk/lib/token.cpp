...
Token::Token(Token **t) :
	...
	_isNull(false),// TSC:from  20140126
	_isArray(false)// TSC:from  20140224
{
}
...
void Token::deleteThis()
{
    if (_next) { // Copy next to this and delete next
        ...
        _link = _next->_link;
        _scope = _next->_scope;
		_isNull=_next->_isNull;
		_isArray=_next->_isArray;
        if (_link)
            _link->link(this);
		...
		_isNull=_previous->_isNull;
		_isArray=_previous->_isArray;
        if (_link)
            _link->link(this);
...
void Token::replace(Token *replaceThis, Token *start, Token *end)
{
    ...
    for (Token *tok = start; tok && tok != end->next(); tok = tok->next())
...
bool Token::Match(const Token *tok, const char pattern[], unsigned int varid)
{
    ...
    while (p && *p) {
        ...
        if (p[0] == '%' && p[1]!='|') {
            ...
            break;
			case 'g': // added by TSC
			//Accept any token (%gol%) while ')'
			{
				p +=5;
				patternUnderstood = true;
				int deep = 1;
				while( !Match(tok,")") && deep != 0)
				{
					if ( Match(tok,"(") )
					{
						deep++;
...
void Token::move(Token *srcStart, Token *srcEnd, Token *newLocation)
{
    ...
    for (Token *tok = srcStart; tok && tok != srcEnd->next(); tok = tok->next())
...
// TSC:from  nextArument2() bug fixed for :fun(a,(char*)b,c)
Token* Token::nextArgument2() const
{
    for (const Token* tok = this; tok; tok = tok->next()) {
        if (tok->str() == ",")
        {
			if(tok->next() && (tok->next()->str()== "(" ||tok->next()->str()== "{"||tok->next()->str()== "["))
			{  tok=tok->next()->link();
			    if(tok==NULL)
				  return NULL;
				else
...
void Token::printOut(const char *title) const
{
    if (title)
        std::cout << "\n### " << title << " ###\n";
    std::cout << stringifyList(true, true, true, true, true, true, 0, 0) << std::endl;
}
...
void Token::printOut(const char *title, const std::vector<std::string> &fileNames) const
{
    if (title)
        std::cout << "\n### " << title << " ###\n";
    std::cout << stringifyList(true, true, true, true, true, true, &fileNames, 0) << std::endl;
}
...
std::string Token::stringifyList(bool varid, bool attributes, bool linenumbers, bool linebreaks, bool files, bool format, const std::vector<std::string>* fileNames, const Token* end) const
{
    if (this == end)
        return "";
    ...
    for (const Token *tok = this; tok && tok != end; tok = tok->next()) {
...
{
    return stringifyList(false, attributes, false, false, false, true, 0, end);
}

std::string Token::stringifyList(bool varid) const
{
    return stringifyList(varid, false, true, true, true, true, 0, 0);
}
