...
void CheckAssert::sideEffectInAssertError(const Token *tok, const std::string& functionName)
{
	reportError(tok, Severity::warning, ErrorType::None,
                ...
                "builds, this is a bug.", ErrorLogger::GenWebIdentity(functionName));
}
...
void CheckAssert::assignmentInAssertError(const Token *tok, const std::string& varname)
{
	reportError(tok, Severity::warning, ErrorType::Suspicious,
		"assignmentInAssert", "Assignment '" + varname + " = xx"+"' will not take effect in release build."
		"Variable '" + varname + "' is modified insert assert statement. "
		"Assert statements are removed from release builds so the code inside "
		"assert statement is not executed. If the code is needed also in release "
		"builds, this is a bug.", ErrorLogger::GenWebIdentity(varname));
}
