...
class TSCANCODELIB CheckStl : public Check {
public:
    ...
    void runSimplifiedChecks(const Tokenizer *tokenizer, const Settings *settings, ErrorLogger *errorLogger) {
		...
		checkStl.checkCompareFunction();
		//checkStl.stlSuspiciousOutOfBoundsWithoutSizeCheck(); //from TSC1.0
	...
	void unAssignedIteratorError(const Token* deref, const std::string &itername);
    /**
...
private:
	/**from TSC1.0*/
	void stlPossibleOutOfBounds(const Token *tok);

	/**from TSC1.0*/
	void stlSuspiciousOutOfBoundsWithoutSizeCheck();

	/**from TSC1.0*/
	void unexpectedIteratorError(const Token *tok, const std::string &iteratorName);
