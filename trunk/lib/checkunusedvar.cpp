        ...
        for (std::list<Variable>::const_iterator i = scope->varlist.begin(); i != scope->varlist.end(); ++i) {
            ...
            if (defValTok && defValTok->str() == "=") {
                if (defValTok->next() && defValTok->next()->str() == "{") {
                    ...
                    doAssignment(variables, i->nameToken(), false, scope);//ignore TSC
            }
        }
    }
...
{
    reportError(tok, Severity::style, ErrorType::None, "unusedVariable", "Unused variable: " + varname, ErrorLogger::GenWebIdentity(varname));
}

void CheckUnusedVar::allocatedButUnusedVariableError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::style, ErrorType::None, "unusedAllocatedMemory", "Variable '" + varname + "' is allocated memory that is never used.", ErrorLogger::GenWebIdentity(varname));
}

void CheckUnusedVar::unreadVariableError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::style, ErrorType::None, "unreadVariable", "Variable '" + varname + "' is assigned a value that is never used.", ErrorLogger::GenWebIdentity(varname));
}

void CheckUnusedVar::unassignedVariableError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::style, ErrorType::None, "unassignedVariable", "Variable '" + varname + "' is not assigned a value.", ErrorLogger::GenWebIdentity(varname));
}
...
void CheckUnusedVar::unusedStructMemberError(const Token *tok, const std::string &structname, const std::string &varname, bool isUnion)
{
    ...
    reportError(tok, Severity::style, ErrorType::None, "unusedStructMember", std::string(prefix) + structname + "::" + varname + "' is never used.",
		ErrorLogger::GenWebIdentity(structname + "::" + varname));
}
