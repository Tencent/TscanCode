...
{
    // eg:a = a + b;
    if (tok && Token::Match(tok, "%var% = %var% !!;") && tok->varId() == tok->tokAt(2)->varId()) {
        return tok->tokAt(2);
    }
                    ...
                    if (dereference)
                        ...
                        if (addressOf ||
                            ...
                            // not in same scope as declaration
                                ...
                                if (scope && var1->_assignments.find(scope) == var1->_assignments.end() ||
    ...
    if (scope->isExecutable()) {
        ...
        for (std::list<Variable>::const_iterator i = scope->varlist.begin(); i != scope->varlist.end(); ++i) {
            ...
            for (; defValTok; defValTok = defValTok->next()) {
                ...
                else if (defValTok && (defValTok->str() == "(" || defValTok->str() == "=")) {
                    variables.addVar(&*i, type, true);
                    break;
                } else if (defValTok && (defValTok->str() == ";" || defValTok->str() == "," || defValTok->str() == ")")) {
            ...
            } else if (defValTok && Token::Match(defValTok, "( %var% )")) // Variables used to initialize the variable read.
...
void CheckUnusedVar::checkFunctionVariableUsage()
{
    ...
    for (std::list<Scope>::const_iterator scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
        ...
        for (Variables::VariableMap::const_iterator it = variables.varUsage().begin(); it != variables.varUsage().end(); ++it) {
            ...
            else if (usage._modified && !usage._write && !usage._allocateMemory && var && !Token::simpleMatch(var->typeStartToken(), "std ::"))
            ...
            else if (!usage._write && !usage._allocateMemory && var && !Token::simpleMatch(var->typeStartToken(), "std ::"))
                unassignedVariableError(usage._var->nameToken(), varname);
        }
    }
}
...
{
    reportError(tok, Severity::style, "unusedStructMember","unusedStructMember", "struct or union member '" + structname + "::" + varname + "' is never used.");
}
