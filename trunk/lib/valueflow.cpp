...
static void bailout(TokenList *tokenlist, ErrorLogger *errorLogger, const Token *tok, const std::string &what)
{
	...
	ErrorLogger::ErrorMessage errmsg(callstack, Severity::debug, "ValueFlow bailout: " + what, ErrorType::None, "valueFlowBailout", false);
                ...
                if (value1->varId == 0U || value2->varId == 0U ||
                    ...
                    if (value1->tokvalue->tokType() == Token::eString) {
                        ...
                        if (index == s.size()) {
                            ...
                            result.intvalue = s[(unsigned)index];
                            setTokenValue(parent, result);
                        }
        ...
        // conditional block of code that assigns variable..
            ...
            for (Token* tok3 = tok2->tokAt(2); tok3 != tok2->next()->link(); tok3 = tok3->next()) {
                if (tok3->varId() == varid)
				{
					...
					if (bAfterCond)
					{
						if (tok2->str() == "if" && tok3->astParent() && Token::Match(tok3->astParent(), "<|<=|>|>="))
						{
							bReturn = true;
						}
					}
    ...
    for (std::size_t i = 0; i < functions; ++i) {
        ...
        for (Token* tok = const_cast<Token*>(scope->classStart); tok != scope->classEnd; tok = tok->next()) {
            ...
            if (!var || !(var->isLocal() || var->isArgument()))
                continue;
            std::list<ValueFlow::Value> values;
			MathLib::bigint  numInCond = 0LL;
			MathLib::bigint  numAfterCond = 0LL;
			bool bIsEqual = false;
			if (numtok)
			{
				numAfterCond = numInCond = MathLib::toLongNumber(numtok->str());
            ...
            values.push_back(ValueFlow::Value(tok,  numInCond));
            ...
            if (top && Token::Match(top->previous(), "if|while (") && !top->previous()->isExpandedMacro()) {
                ...
                if (Token::simpleMatch(top->link(), ") {")) {
                    ...
                    if (!isreturn) {
						...
						if (numtok)
						{
							if (isReturn(after) && isOpposite)
							{

							}
							else
							{
								values.push_back(ValueFlow::Value(tok, numAfterCond));
							}
						}
                        valueFlowForward(after->next(), top->scope()->classEnd, var, varid, values, constValue, tokenlist, errorLogger, settings, true);//ignore TSC
                    }
					else
					{
						if (numtok && !bIsEqual)
						{
							values.clear();
							values.push_back(ValueFlow::Value(tok, numAfterCond));
							valueFlowForward(after->next(), top->scope()->classEnd, var, varid, values, false, tokenlist, errorLogger, settings, true);//ignore TSC
						}
    ...
    if (!expr)
        ...
        *result = !(*result);
        ...
        if (index >= 0 && index < strValue.size())
            *result = strValue[(unsigned)index];
    ...
    for (std::list<Scope>::iterator scope = symboldatabase->scopeList.begin(); scope != symboldatabase->scopeList.end(); ++scope) {
        ...
        for (Token *tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next()) {
            ...
            if (Token::Match(tok, "case %num% :")) {
				...
				if (vartok->variable() && vartok->variable()->scope())
				{
