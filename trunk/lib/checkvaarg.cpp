...
void CheckVaarg::wrongParameterTo_va_start_error(const Token *tok, const std::string& paramIsName, const std::string& paramShouldName)
{
    reportError(tok, Severity::warning, ErrorType::None,
		"va_start_wrongParameter", "'" + paramIsName + "' given to va_start() is not last named argument of the function. Did you intend to pass '" + paramShouldName + "'?", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckVaarg::referenceAs_va_start_error(const Token *tok, const std::string& paramName)
{
    reportError(tok, Severity::error, ErrorType::None,
		"va_start_referencePassed", "Using reference '" + paramName + "' as parameter for va_start() results in undefined behaviour.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
    reportError(tok, Severity::error, ErrorType::None,
		"va_end_missing", "va_list '" + varname + "' was opened but not closed by va_end().", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckVaarg::va_list_usedBeforeStartedError(const Token *tok, const std::string& varname)
{
    reportError(tok, Severity::error, ErrorType::None,
		"va_list_usedBeforeStarted", "va_list '" + varname + "' used before va_start() was called.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckVaarg::va_start_subsequentCallsError(const Token *tok, const std::string& varname)
{
    reportError(tok, Severity::error, ErrorType::None,
		"va_start_subsequentCalls", "va_start() or va_copy() called subsequently on '" + varname + "' without va_end() in between.", ErrorLogger::GenWebIdentity(tok->str()));
}
