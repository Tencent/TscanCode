    ...
    for (std::size_t i = 0; i < classes; ++i) {
        ...
        // TODO: handle union variables better
        {
            ...
            for (std::list<Scope *>::const_iterator it = scope->nestedList.begin(); it != scope->nestedList.end(); ++it) {
                ...
                if (nestedScope && nestedScope->type == Scope::eUnion) {
                    bailout = true;
                    break;
                }
            }
		...
		//remove variables which repeat by name 
		unsigned int count_var_real=0;
		std::set<string> set_var;
		std::list<Variable>::const_iterator var_all;
		for (var_all = scope->varlist.begin(); var_all != scope->varlist.end(); ++var_all)
		{
			set_var.insert((var_all->nameToken())->str());
		}
		count_var_real = set_var.size();
		std::vector<Usage> usage(count_var_real);
		//end  TSC 20140812
        ...
        for (func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
			...
			// TSC:from 20140512 if construct func is null,not report error
			const Token* mftok=func->tokenDef;
			if(mftok && mftok->next() && mftok->next()->str()=="(")
			{
				mftok=mftok->next();
				mftok=mftok->link();
				if(mftok && mftok->str()==")")
			...
			// TSC:from TSC 20140512
			std::string varname="";
			 bool inconclusive = false;
            for (var = scope->varlist.begin(); var != scope->varlist.end(); ++var, ++count) {
			   inconclusive = false;
			   //from  20140812
			   //skip those repeat variables
			   if(count>=count_var_real)
				   continue;
                ...
                if (func->type == Function::eOperatorEqual) {
                    ...
                    for (const Token *operTok = operStart; operStart && operTok && operTok != operStart->link(); operTok = operTok->next()) {
                    ...
                    if (!varType || varType->type != Scope::eUnion)
					{
						...
						// TSC:from TSC 20140512 one construct func only one error ,one error include all not inited membervars 
                       // uninitVarError(func->token, scope->className, var->name(), inconclusive);
					}
                }
            }

			vector<Usage>::iterator Usageitr;
			unsigned int all=0;
			...
			if(all>0&&all<usage.size()&&varname!=""&&getCheckConfig()->logic&&getCheckConfig()->uninitMemberVar)
			{
...
void CheckClass::noCopyConstructorError(const Token *tok, const std::string &classname, bool isStruct)
{
	...
	UNREFERENCED_PARAMETER(isStruct);
}
...
void CheckClass::assignVar(const std::string &varname, const Scope *scope, std::vector<Usage> &usage)
{
	...
	unsigned int maxcount=usage.size(); 
	for (var = scope->varlist.begin(); var != scope->varlist.end(); ++var, ++count) {
		if (var->name() == varname ){
			if(count<maxcount)
			{
        ...
        // Assignment of array item of member variable?
            ...
            while (tok2) {
                if (Token::simpleMatch(tok2->next(), "["))
                    tok2 = tok2->next()->link();
                else if (tok2 && Token::Match(tok2->next(), ". %var%"))
                    tok2 = tok2->tokAt(2);
                else
                    break;
            }
...
{
	reportError(tok, Severity::error, "logic","uninitMemberVar", "Member variable '" + classname + "::" + varname + "' is not initialized in the constructor.", inconclusive);
}
...
static bool checkFunctionUsage(const std::string& name, const Scope* scope)
{
    ...
    for (std::list<Function>::const_iterator func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
        if (func->functionScope) {
            for (const Token *ftok = func->functionScope->classStart; ftok && ftok != func->functionScope->classEnd; ftok = ftok->next()) {
...
void CheckClass::noMemset()
{
    ...
    for (std::size_t i = 0; i < functions; ++i) {
        ...
        for (const Token *tok = scope->classStart; tok && tok != scope->classEnd; tok = tok->next()) {
            ...
            if (arg3 && Token::Match(arg3, "sizeof ( %type% ) )"))
                typeTok = arg3->tokAt(2);
            else if (arg3 && Token::Match(arg3, "sizeof ( %type% :: %type% ) )"))
                typeTok = arg3->tokAt(4);
            else if (arg3 && Token::Match(arg3, "sizeof ( struct %type% ) )"))
                typeTok = arg3->tokAt(3);
            else if (arg1 && Token::Match(arg1, "&| %var% ,")) {
...
void CheckClass::checkMemsetType(const Scope *start, const Token *tok, const Scope *type)
{
    ...
    for (std::size_t i = 0; type && i < type->derivedFrom.size(); i++) {
    ...
    for (var = type->varlist.begin(); var != type->varlist.end(); ++var) {
        ...
        if (!var->isStatic() && !var->isConst() && !var->isPointer() && !var->isReference()) {
            ...
            // check for std:: type
            if (tok1 && Token::simpleMatch(tok1, "std ::"))
                memsetError(tok, tok->str(), "'std::" + tok1->strAt(2) + "'", type->classDef->str());

            // check for known type
			//modify by TSC solve stack overflow in recursion
            //else if (var->type())
			else if(var->type()!=type && var->type())
...
{
	UNREFERENCED_PARAMETER(tok);
	UNREFERENCED_PARAMETER(memfunc);
	UNREFERENCED_PARAMETER(classname);
    ...
    for (std::size_t i = 0; i < classes; ++i) {
        ...
        for (func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
            if (func->type == Function::eOperatorEqual && func->hasBody) {
                ...
                while (typeTok && (typeTok->str() == "const" || typeTok->str() == "&" || typeTok->str() == "*"))
                    typeTok = typeTok->previous();
                if (typeTok && typeTok->str() != scope->className)
                    continue;
...
{
    reportError(tok, Severity::warning, "checkClass","operatorEqToSelf",
...
{
#ifdef WINGUI
	reportError(tok, Severity::performance, "memleak","virtualDestructor", "Class '" + Base + "' which is inherited by class '" + Derived + "' does not have a virtual destructor.\n"
...
#else
    reportError(tok, Severity::error, "memleak","virtualDestructor", "Class '" + Base + "' which is inherited by class '" + Derived + "' does not have a virtual destructor.\n"
                "Class '" + Base + "' which is inherited by class '" + Derived + "' does not have a virtual destructor. "
                "If you destroy instances of the derived class by deleting a pointer that points to the base class, only "
                "the destructor of the base class is executed. Thus, dynamic memory that is managed by the derived class "
                "could leak. This can be avoided by adding a virtual destructor to the base class.");
...
void CheckClass::thisSubtractionError(const Token *tok)
{
    ...
    reportError(tok, Severity::portability, "Suspicious", "thisSubtraction","Suspicious pointer subtraction. Did you intend to write '->'?");
#else
        ...
        for (func = scope->functionList.begin(); func != scope->functionList.end(); ++func) {
            ...
            if (func->type == Function::eFunction && func->hasBody && !func->isFriend && !func->isStatic && !func->isVirtual) {
                ...
                if (Token::Match(previous, "*|&")) {
                    ...
                    while (temp && !Token::Match(temp->previous(), ";|}|{|public:|protected:|private:"))
                        temp = temp->previous();

                    if (temp && temp->str() != "const")
                        continue;
                } else if (previous && Token::Match(previous->previous(), "*|& >")) {
                    const Token *temp = previous;
                    ...
                    // LPVOID, HDC, etc
                    if (previous && previous->isUpperCaseName() && previous->str().size() > 2 && !symbolDatabase->isClassOrStruct(previous->str()))
                        continue;
                }
    ...
    for (const Token *tok1 = func->functionScope->classStart; tok1 && tok1 != func->functionScope->classEnd; tok1 = tok1->next()) {
        ...
        // function call..
            ...
            for (const Token* tok2 = tok1->tokAt(2); tok2 && tok2 != tok1->next()->link(); tok2 = tok2->next()) {
                if (tok2->str() == "(")
                    tok2 = tok2->link();
                else if (tok2 && tok2->isName() && isMemberVar(scope, tok2)) {
...
{
	UNREFERENCED_PARAMETER(funcname);
	UNREFERENCED_PARAMETER(suggestStatic);
	UNREFERENCED_PARAMETER(classname);
...
void CheckClass::initializerListError(const Token *tok1, const Token *tok2, const std::string &classname, const std::string &varname)
{
    ...
    toks.push_back(tok1);
    toks.push_back(tok2);
    reportError(toks, Severity::style,  "checkClass","initializerList",
