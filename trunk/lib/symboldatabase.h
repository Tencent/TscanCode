...
public:
	...
	//add by TSC 20140913 get class name;
...
public:
	...
	//TSC:from TSC 20140912
	std::string returnVarType(const Variable *var) const;

	std::vector<std::string> returnArgsTypeList(const Token *tok) const;

	std::vector<Variable*> returnArgsVariableList(const Token *tok) const;

	std::vector<Token*> returnArgsTokenList(const Token *tok) const;
	...
	const Scope* findFunctionScopeByName(const std::string& name) const; // added by kyle
...
private:
	...
	// TSC:from TSC 20140408 set token's funcinfo
	void setFunctionInfoForToken(const Token* tok, const Token* argStart); // TSC:from TSC 0408 set token's funcinfo

	// TSC:from TSC 20140913 set token's funcinfo for class function without function declaration
	void setFunctionInfoForToken(const Token* tok, const Token* argStart,std::string className); // TSC:from TSC 0408 set token's funcinfo
