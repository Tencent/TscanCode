...
void CheckBool::incrementBooleanError(const Token *tok)
{
    reportError(
        tok,
		Severity::style, ErrorType::Compute, 
        "incrementboolean",
...
void CheckBool::checkBitwiseOnBoolean()
{
    ...
    for (std::size_t i = 0; i < functions; ++i) {
        ...
        for (const Token* tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next()) {
			...
			else if (Token::Match(tok, "(|.|return|&&|%oror%|throw|, true|false [&|]")) {	
					bitwiseOnBooleanError(tok->next(), tok->next()->str(), tok->strAt(2) == "&" ? "&&" : "||");
					tok = tok->tokAt(2);	
			}
			else if (Token::Match(tok, "[&|] true|false )|.|return|&&|%oror%|throw|,") && (!tok->previous() || !tok->previous()->isExtendedOp() || tok->strAt(-1) == ")" || tok->strAt(-1) == "]")) {
				
					bitwiseOnBooleanError(tok->next(), tok->next()->str(), tok->str() == "&" ? "&&" : "||");
					tok = tok->tokAt(2);
...
{
    reportError(tok, Severity::style, ErrorType::Logic, "bitwiseOnBoolean",
...
{
    if (n0o1)
		reportError(tok, Severity::warning, ErrorType::Logic, "ComparisonOfBoolWithInt",
					...
					"neither 1 nor 0.", ErrorLogger::GenWebIdentity(tok->str()));
    else
		reportError(tok, Severity::warning, ErrorType::Logic, "ComparisonOfBoolWithInt",
                    "Comparison of a boolean with an integer.\n"
                    "The expression '" + expression + "' is of type 'bool' "
					"and it is compared against an integer value.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckBool::comparisonOfBoolWithInvalidComparator(const Token *tok, const std::string &expression)
{
	reportError(tok, Severity::warning, ErrorType::None, "ComparisonOfBoolWithInvalidComparator",
				...
				" operator could cause unexpected results.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
	reportError(tok, Severity::style, ErrorType::None, "comparisonOfFuncReturningBoolError",
...
{
	reportError(tok, Severity::style, ErrorType::None, "comparisonOfTwoFuncsReturningBoolError",
...
bool CheckBool::IsBoolExpression(const Token* tok)
{
	if (isBool(tok->variable()) || Token::Match(tok, "<|<=|>|>=|==|!=|&&|%oror%"))
		return true;

	else if (tok->str() == "(")
	{
		if (const Token* tokFunc = tok->previous())
		{
			if (tokFunc->isName())
			{
				...
				if (gtFunc)
				{
					const std::vector<std::string>& returnVec = gtFunc->GetReturn();
					if(!returnVec.empty())
						return (*returnVec.rbegin()) == "bool";
				}
			}
		}
	}
...
void CheckBool::checkComparisonWithBool()
{
	const SymbolDatabase* const symbolDatabase = _tokenizer->getSymbolDatabase();
	...
	for (std::size_t i = 0; i < functions; ++i)
	{
		...
		for (const Token* tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next())
		{
			...
			if (Token::Match(tok, "<|<=|>|>=") && !tok->link() && tok1 && tok2)
			{
				// fix fp: read@100<T1>,read@100<T1*>, 'read' was tokenized as variable incorrectly
				if (tok->str() == ">")
				{
					const Token* tok3 = tok->previous();
					while (tok3 && Token::Match(tok3, "%name%|*"))
					{
						if (tok3->isName() && tok3->varId())
						{
							tok3 = nullptr;
							break;
						}
				...
				bool bError = false;
				if (IsBoolExpression(tok1))
				{
					comparisonWithBoolError(tok, tok1->astString());
					bError = true;
				}

				if (!bError)
				{
					if (IsBoolExpression(tok2))
						comparisonWithBoolError(tok, tok2->astString());
				}
			}
		}
	}
...
void CheckBool::comparisonWithBoolError(const Token *tok, const std::string &expression)
{
	reportError(tok, Severity::style, ErrorType::Logic, "ComparisonWithBool",
		"Comparison of a variable having boolean value using relational (<, >, <= or >=) operator.\n"
		"The expression '" + expression + "' is of type 'bool' "
		"and comparing 'bool' value using relational (<, >, <= or >=)"
		" operator could cause unexpected results.", expression);
}

void CheckBool::comparisonOfBoolWithBoolError(const Token *tok, const std::string &expression)
{
	reportError(tok, Severity::style, ErrorType::Logic, "ComparisonOfBoolWithBool",
                "Comparison of a variable having boolean value using relational (<, >, <= or >=) operator.\n"
    ...
    for (std::size_t i = 0; i < functions; ++i) {
        ...
        for (const Token* tok = scope->classStart; tok != scope->classEnd; tok = tok->next()) {
            if (tok->str() == "=" && astIsBool(tok->astOperand2())) {
				...
				assignBoolToPointerError(tok->astOperand2());
            }
        }
    }
}
...
{
	reportError(tok, Severity::warning, ErrorType::Suspicious, "assignBoolToPointer",
		"Boolean value assigned to pointer.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckBool::comparisonOfBoolExpressionWithIntError(const Token *tok, bool n0o1)
{
    ...
    if (n0o1)
		reportError(tok, Severity::warning, ErrorType::Logic, "ComparisonOfBoolWithInt",
		"Comparison of a boolean expression with an integer other than 0 or 1.", ErrorLogger::GenWebIdentity(tok->str()));
    else
		reportError(tok, Severity::warning, ErrorType::Logic, "ComparisonOfBoolWithInt",
		"Comparison of a boolean expression with an integer.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckBool::pointerArithBoolError(const Token *tok)
{
    reportError(tok,
		Severity::warning,
				...
				"Converting pointer arithmetic result to bool. The boolean result is always true unless there is pointer arithmetic overflow, and overflow is undefined behaviour. Probably a dereference is forgotten.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
	reportError(tok, Severity::style, ErrorType::None, "assignBoolToFloat",
		"Boolean value assigned to floating point variable.", ErrorLogger::GenWebIdentity(tok->str()));
}
