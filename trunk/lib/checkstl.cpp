...
{
    reportError(tok, Severity::portability,"Suspicious", "invalidIterator", "Invalid iterator: " + iteratorName);
}

void CheckStl::iteratorsError(const Token *tok, const std::string &container1, const std::string &container2)
{
    reportError(tok, Severity::portability, "Suspicious","invalidIterator", "Same iterator is used with different containers '" + container1 + "' and '" + container2 + "'.");
}
//add by TSC 20141027
void CheckStl::unAssignedIteratorError(const Token* deref, const std::string &itername)
{
	reportError(deref, Severity::portability, "Suspicious","invalidIterator",
...
void CheckStl::dereferenceErasedError(const Token *erased, const Token* deref, const std::string &itername)
{
    if (erased) {
        ...
        reportError(callstack, Severity::portability,"Suspicious", "invalidIterator",
    ...
    for (unsigned int iteratorId = 1; iteratorId < symbolDatabase->getVariableListSize(); iteratorId++) {
        ...
        for (const Token *tok2 = var->nameToken(); tok2 != var->scope()->classEnd; tok2 = tok2->next()) {
            ...
            // Dereferencing invalid iterator?
				...
				{
					unAssignedIteratorError(tok2,tok2->strAt(1));
				}
                tok2 = tok2->next();
...
{
	reportError(tok, Severity::portability,"Suspicious", "invalidIterator", "Iterators of different containers are used together.");
}
...
void CheckStl::mismatchingContainers()
{
    ...
    for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (Token::Match(arg1, pattern2.c_str()) && algorithm2.find(tok->strAt(2)) != algorithm2.end()) {
            ...
            if (arg3 && Token::Match(arg3, pattern2.c_str()) && arg3->str() != arg3->strAt(6))
            ...
            if (arg1 && arg3 && Token::Match(arg3, pattern1x1_2.c_str())) {
...
{
    if (at)
        reportError(tok, Severity::warning,"bufoverrun", "stlOutOfBounds", "When " + num + "==" + var + ".size(), " + var + ".at(" + num + ") is out of bounds.");
    else
        reportError(tok, Severity::warning, "bufoverrun","stlOutOfBounds", "When " + num + "==" + var + ".size(), " + var + "[" + num + "] is out of bounds.");
}
...
{
    reportError(tok, Severity::error, "checkstl","invalidIterator2", "After " + func + "(), the iterator '" + iterator_name + "' may be invalid.");
}
...
{
    reportError(tok, Severity::error,"checkstl", "invalidPointer", "Invalid pointer '" + pointer_name + "' after " + func + "().");
}
...
{
    reportError(tok, Severity::error,"checkstl", "stlBoundries",
...
void CheckStl::if_findError(const Token *tok, bool str)
{
    ...
    if (str)
		reportError(tok, Severity::portability,"Suspicious", "invalidIterator",
		...
		reportError(tok, Severity::portability,"Suspicious", "invalidIterator", "Suspicious condition. The result of find() is an iterator, but it is not properly checked.");
...
void CheckStl::sizeError(const Token *tok)
{
    ...
    reportError(tok, Severity::performance,"checkstl", "stlSize",
...
{
    reportError(tok, Severity::style,"checkstl", "redundantIfRemove",
    ...
    for (std::list<Scope>::const_iterator i = symbolDatabase->scopeList.begin(); i != symbolDatabase->scopeList.end(); ++i) {
        ...
        for (const Token *tok2 = i->classDef->tokAt(2); tok2 != i->classStart; tok2 = tok2->next()) {
            ...
            for (const Token *tok3 = i->classStart; tok3 && tok3 != i->classEnd; tok3 = tok3->next()) {
                if (Token::Match(tok3, "%varid% ++", iteratorId)&&!Token::simpleMatch(tok3->tokAt(-2), "erase ("))
                    incrementToken = tok3;
...
void CheckStl::missingComparisonError(const Token *incrementToken1, const Token *incrementToken2)
{
    ...
    reportError(callstack, Severity::warning, "bufoverrun","StlMissingComparison", errmsg.str());
}
    ...
    for (std::list<Scope>::const_iterator scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
        ...
        for (const Token *tok = scope->classStart; tok && tok != scope->classEnd; tok = tok->next()) {
           ...
           // else if ((returnType == stdString || returnType == stdStringConstRef) && _settings->isEnabled("performance")) {
			 else if ((returnType == stdString || returnType == stdStringConstRef) ) {
                if (tok->str() == "return") {
...
{
    reportError(tok, Severity::error,"checkstl", "stlcstrthrow", "Dangerous usage of c_str(). The value returned by c_str() is invalid after throwing exception.\n"
...
{
    reportError(tok, Severity::error,"checkstl", "stlcstr", "Dangerous usage of c_str(). The value returned by c_str() is invalid after this call.\n"
...
{
    reportError(tok, Severity::performance,"checkstl", "stlcstrReturn", "Returning the result of c_str() in a function that returns std::string is slow and redundant.\n"
...
void CheckStl::string_c_strParam(const Token* tok, unsigned int number)
{
    ...
    reportError(tok, Severity::performance,"checkstl", "stlcstrParam", oss.str());
}
...
{
    reportError(tok, Severity::style, "checkstl","useAutoPointerCopy",
...
{
    reportError(tok, Severity::error,"checkstl", "useAutoPointerContainer",
...
{
    reportError(tok, Severity::error,"checkstl", "useAutoPointerArray",
...
void CheckStl::uselessCalls()
{
    ...
    for (const Token *tok = _tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        } else if (Token::Match(tok, "[{}:;] %var% . empty ( ) ;") )
...
void CheckStl::uselessCallsReturnValueError(const Token *tok, const std::string &varname, const std::string &function)
{
    ...
    reportError(tok, Severity::warning,"checkstl", "uselessCallsCompare", errmsg.str());
}
...
void CheckStl::uselessCallsSwapError(const Token *tok, const std::string &varname)
{
    ...
    reportError(tok, Severity::performance,"checkstl", "uselessCallsSwap", errmsg.str());
}

void CheckStl::uselessCallsSubstrError(const Token *tok, bool empty)
{
    if (empty)
        reportError(tok, Severity::performance,"checkstl", "uselessCallsSubstr", "Ineffective call of function 'substr' because it returns an empty string.");
    else
        reportError(tok, Severity::performance,"checkstl", "uselessCallsSubstr", "Ineffective call of function 'substr' because it returns a copy of the object. Use operator= instead.");
}

void CheckStl::uselessCallsEmptyError(const Token *tok)
{
    reportError(tok, Severity::warning, "checkstl","uselessCallsEmpty", "Ineffective call of function 'empty()'. Did you intend to call 'clear()' instead?");
}

void CheckStl::uselessCallsRemoveError(const Token *tok, const std::string& function)
{
    reportError(tok, Severity::warning,"checkstl", "uselessCallsRemove", "Return value of std::" + function + "() ignored. Elements remain in container.\n"
