...
{
    reportError(tok, Severity::error, ErrorType::Suspicious, "invalidIterator", "Invalid iterator: " + iteratorName, ErrorLogger::GenWebIdentity(iteratorName));
}

void CheckStl::iteratorsError(const Token *tok, const std::string &container1, const std::string &container2)
{
    reportError(tok, Severity::error, ErrorType::Suspicious, "invalidIterator", 
		"Same iterator is used with different containers '" + container1 + "' and '" + container2 + "'.", ErrorLogger::GenWebIdentity(container1 +","+ container2));
}

void CheckStl::unAssignedIteratorError(const Token* deref, const std::string &itername)
{
	reportError(deref, Severity::error, ErrorType::Suspicious, "invalidIterator",
		"Invalid iterator '" + itername + "' used."
		"The iterator '" + itername + "' is invalid before being assigned. "
		"Dereferencing or comparing it with another iterator is invalid operation.", ErrorLogger::GenWebIdentity(deref->str()));
...
void CheckStl::dereferenceErasedError(const Token *erased, const Token* deref, const std::string &itername)
{
    ...
    if (erased) {
		...
		const Variable* var = symbolDatabase->getVariableFromVarId(deref->varId());
		if (var && !Token::Match(var->typeStartToken(), "std| ::| vector|deque"))
		{
			return;
		}
		...
		//reportError(deref, Severity::error, ErrorType::None, "eraseDereference",
		reportError(callstack, Severity::error, ErrorType::Suspicious, "invalidIterator",
                    ...
                    "Dereferencing or comparing it with another iterator is invalid operation.", ErrorLogger::GenWebIdentity(itername));
    } else {
		reportError(deref, Severity::error, ErrorType::Suspicious, "invalidIterator",
                    "Invalid iterator '" + itername + "' used.\n"
                    "The iterator '" + itername + "' is invalid before being assigned. "
...
void CheckStl::iterators()
{
    ...
    for (unsigned int iteratorId = 1; iteratorId < symbolDatabase->getVariableListSize(); iteratorId++) {
        ...
        if (!var || !var->isLocal() || !Token::Match(var->typeEndToken(), "iterator|const_iterator|reverse_iterator|const_reverse_iterator|auto"))
            continue;
		//排除掉自定义的iterator
		if (!Token::Match(var->typeEndToken()->previous(), "::"))
		{
			continue;
		}
        ...
        for (const Token *tok2 = var->nameToken(); tok2 && tok2 != var->scope()->classEnd; tok2 = tok2->next()) {
			...
			if (tok2 && Token::Match(tok2, "%varid% !=|== %name% . end|rend|cend|crend ( )", iteratorId) && container && tok2->tokAt(2)->varId() != container->declarationId()) {
				iteratorsError(tok2, container->name(), tok2->strAt(2));
				tok2 = tok2->tokAt(6);
			}
			else if (Token::Match(tok2, ". erase ( ++ %varid%", iteratorId) || Token::Match(tok2, ". erase ( %varid% ++", iteratorId))
			{
				//eg.std::map<uint32_t, std::vector<struct _Relation> >::iterator relationIter; can match "vector"
				//if (Token::findsimplematch(var->typeStartToken(), "vector", var->typeEndToken()))
				//only vector,deque report error
				if (Token::Match(var->typeStartToken(), "std| ::| vector|deque"))
				{
					const Token* tokI = tok2->tokAt(3);
					if (tokI->str() == "++")
					{
						tokI = tokI->next();
					}
					unexpectedIteratorError(tokI, tokI->str());
				}
			}
			// Is the iterator used in a insert/erase operation?
                ...
                if (container && tok2->varId() != container->declarationId()) {
                    ...
                    if (Token::simpleMatch(decltok, "const| std :: set"))
                        continue; // No warning
...
{
	reportError(tok, Severity::error, ErrorType::Suspicious, "invalidIterator", "Iterators of different containers are used together.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
    if (at)
        reportError(tok, Severity::error, ErrorType::BufferOverrun, "stlOutOfBounds", "When " + num + "==" + var + ".size(), " + var + ".at(" + num + ") is out of bounds.", ErrorLogger::GenWebIdentity(var));
    else
        reportError(tok, Severity::error, ErrorType::BufferOverrun, "stlOutOfBounds", "When " + num + "==" + var + ".size(), " + var + "[" + num + "] is out of bounds.", ErrorLogger::GenWebIdentity(var));
}
		...
		for (std::list<ExecutionPath *>::const_iterator it = checks.begin(); it != checks.end(); ++it) {
			...
			if (c && c->eraseToken) {
				...
				if (checkStl) {
					...
					{
						checkStl->unAssignedIteratorError(c->useToken, c->useToken->str());
					}
				}
			}
		}
...
void CheckStl::erase2()
{
	...
	for (std::list<Scope>::const_iterator i = symbolDatabase->scopeList.begin(); i != symbolDatabase->scopeList.end(); ++i) {
		...
		else if (tok && i->type == Scope::eWhile && Token::Match(tok, "while ( %var% !=")) {
...
void CheckStl::eraseCheckLoopVar(const Scope &scope, const Variable *var)
{
    ...
    for (const Token *tok = scope.classStart; tok != scope.classEnd; tok = tok->next()) {
        ...
        for (; tok2 != scope.classEnd; tok2 = tok2->next()) {
            ...
            if (indentlevel == 0U && Token::Match(tok2, "break|return|goto|continue|abort|exit|ExitProcess"))
                break;
        }
...
{
    reportError(tok, Severity::error, ErrorType::Suspicious, "invalidIterator",
		"After " + func + "(), the iterator '" + iterator_name + "' may be invalid.", ErrorLogger::GenWebIdentity(iterator_name));
}
...
{
	reportError(tok, Severity::error, ErrorType::None, "invalidPointer",
		"Invalid pointer '" + pointer_name + "' after " + func + "().", ErrorLogger::GenWebIdentity(pointer_name));
}
...
{
    reportError(tok, Severity::error, ErrorType::None, "stlBoundaries",
				...
				"One should use operator!= instead to compare iterators.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
    if (str)
        reportError(tok, Severity::performance, ErrorType::Logic, "stlIfFind",
					...
					"you should compare with std::string::npos.", ErrorLogger::GenWebIdentity(tok->str()));
    else
		reportError(tok, Severity::warning, ErrorType::Logic, "stlIfFind", "Suspicious condition. The result of find() is an iterator, but it is not properly checked.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckStl::sizeError(const Token *tok)
{
    ...
    reportError(tok, Severity::performance, ErrorType::None, "stlSize",
				...
				"guaranteed to take constant time.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
    reportError(tok, Severity::style, ErrorType::None, "redundantIfRemove",
                "Redundant checking of STL container element existence before removing it.\n"
                "Redundant checking of STL container element existence before removing it. "
				"It is safe to call the remove method on a non-existing element.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckStl::missingComparisonError(const Token *incrementToken1, const Token *incrementToken2)
{
    ...
    reportError(callstack, Severity::warning, ErrorType::Logic, "StlMissingComparison", errmsg.str());
}
...
{
    reportError(tok, Severity::error, ErrorType::None, "stlcstrthrow", "Dangerous usage of c_str(). The value returned by c_str() is invalid after throwing exception.\n"
		"Dangerous usage of c_str(). The string is destroyed after the c_str() call so the thrown pointer is invalid.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckStl::string_c_strError(const Token* tok)
{
    reportError(tok, Severity::error, ErrorType::None, "stlcstr", "Dangerous usage of c_str(). The value returned by c_str() is invalid after this call.\n"
		"Dangerous usage of c_str(). The c_str() return value is only valid until its string is deleted.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckStl::string_c_strReturn(const Token* tok)
{
    reportError(tok, Severity::performance, ErrorType::None, "stlcstrReturn", "Returning the result of c_str() in a function that returns std::string is slow and redundant.\n"
		"The conversion from const char* as returned by c_str() to std::string creates an unnecessary string copy. Solve that by directly returning the string.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckStl::string_c_strParam(const Token* tok, unsigned int number)
{
	...
	reportError(tok, Severity::performance, ErrorType::None, "stlcstrParam", oss.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
    reportError(tok, Severity::style, ErrorType::None, "useAutoPointerCopy",
...
{
    reportError(tok, Severity::error, ErrorType::None, "useAutoPointerContainer",
...
{
    reportError(tok, Severity::error, ErrorType::None, "useAutoPointerArray",
...
void CheckStl::autoPointerMallocError(const Token *tok, const std::string& allocFunction)
{
	...
	reportError(tok, Severity::error, ErrorType::None, "useAutoPointerMalloc", summary + "\n" + verbose, ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckStl::uselessCallsReturnValueError(const Token *tok, const std::string &varname, const std::string &function)
{
	...
	reportError(tok, Severity::warning, ErrorType::None, "uselessCallsCompare", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckStl::uselessCallsSwapError(const Token *tok, const std::string &varname)
{
	...
	reportError(tok, Severity::performance, ErrorType::None, "uselessCallsSwap", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckStl::uselessCallsSubstrError(const Token *tok, bool empty)
{
    if (empty)
		reportError(tok, Severity::performance, ErrorType::None, "uselessCallsSubstr", "Ineffective call of function 'substr' because it returns an empty string.", ErrorLogger::GenWebIdentity(tok->str()));
    else
		reportError(tok, Severity::performance, ErrorType::None, "uselessCallsSubstr", "Ineffective call of function 'substr' because it returns a copy of the object. Use operator= instead.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckStl::uselessCallsEmptyError(const Token *tok)
{
	reportError(tok, Severity::warning, ErrorType::None, "uselessCallsEmpty", "Ineffective call of function 'empty()'. Did you intend to call 'clear()' instead?", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckStl::uselessCallsRemoveError(const Token *tok, const std::string& function)
{
    reportError(tok, Severity::warning, ErrorType::None, "uselessCallsRemove", "Return value of std::" + function + "() ignored. Elements remain in container.\n"
				...
				"Elements past new end remain valid but with unspecified values. Use the erase method of the container to delete them.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
{
    reportError(deref, Severity::warning, ErrorType::None,
                "derefInvalidIterator", "Possible dereference of an invalid iterator: " + iterName + "\n" +
				"Make sure to check that the iterator is valid before dereferencing it - not after.", ErrorLogger::GenWebIdentity(deref->str()));
}
...
void CheckStl::checkCompareFunction()
{
	const SymbolDatabase* const symbolDatabase = _tokenizer->getSymbolDatabase();
	...
	for (std::vector<const Scope *>::const_iterator it = symbolDatabase->functionScopes.begin(); it != symbolDatabase->functionScopes.end(); it++)
	{
		std::set<int> vecVarTokens;
		std::set<int> vecVarCheckedSize;

		//use must be after notice;
		const Token* const tok_start = (*it)->classStart;
		const Token* const tok_end = (*it)->classEnd;

		for (const Token *tok = tok_start; tok && tok != tok_end; tok = tok->next())
		{
			if (Token::Match(tok, "std| ::| sort ("))
			{
				tok = Token::findsimplematch(tok, "sort");
				tok = tok->next();

				if (tok->astOperand2() && tok->astOperand2()->str() == ",")
				{
					tok = tok->astOperand2()->astOperand2();
					if (tok && tok->isName())
					{
						for (std::vector<const Scope *>::const_iterator it2 = symbolDatabase->functionScopes.begin(); it2 != symbolDatabase->functionScopes.end(); it2++)
						{
							const Scope* scope = *it2;
							if (scope->className == tok->str())
							{
								for (const Token* tok2 = scope->classStart; tok2 && tok2 != scope->classEnd; tok2 = tok2->next())
								{
									if (tok2->str() == "return")
									{
										if (tok2->astOperand1() && Token::Match(tok2->astOperand1(), ">=|<="))
										{
											reportError(tok, Severity::warning, ErrorType::Logic, "stlSortCheck", "The compare function may return true when two operators equals.", ErrorLogger::GenWebIdentity(tok->str()));
										}
									}
								}
							}
...
{
    reportError(tok, Severity::style, ErrorType::None, "reademptycontainer", "Reading from empty STL container '" + (tok ? tok->str() : std::string("var")) + "'", 0U, true);
}

#pragma region From TSC 1.0

//Add stl vector sizeOf check
//20150611 
//void bla(std::vector<int> &v)
// v[10] = 1; // shoud have checked like "if (v.size()>10)
//default close
void CheckStl::stlSuspiciousOutOfBoundsWithoutSizeCheck()
{
	const SymbolDatabase* const symbolDatabase = _tokenizer->getSymbolDatabase();
	//for (std::list<Scope>::const_iterator it = symbolDatabase->scopeList.begin(); it!= symbolDatabase->scopeList.end(); ++it) {
	// Scan through all tokens..
	for (std::vector<const Scope *>::const_iterator it = symbolDatabase->functionScopes.begin(); it != symbolDatabase->functionScopes.end(); it++)
	{
		...
		//use must be after notice;
		const Token* const tok = (*it)->classStart;
		const Token* const tok_end = (*it)->classEnd;
		for (const Token *tok2 = tok->tokAt(2); tok2 && tok2 != tok_end; tok2 = tok2->next())
		{
			if (tok2->varId() != 0)
			{
				unsigned int cur_varId = tok2->varId();
				const Variable *container = symbolDatabase->getVariableFromVarId(cur_varId);
				if (!container)
					continue;
				if (!Token::simpleMatch(container->typeStartToken(), "std :: vector <"))
					continue;
				else if (vecVarTokens.find(cur_varId) == vecVarTokens.end())
				{
					//const Token *ttttok=container->typeStartToken();
					vecVarTokens.insert(cur_varId);
				}
			}
			if (Token::Match(tok2, "%var% [ "))  //
			{
				Token *tmpTok = tok2->tokAt(1)->link();
				if (!Token::Match(tmpTok, "] ;")) //not  declare;
				{
					unsigned int cur_varId = tok2->varId();
					bool bOverRunMaxIndex = true;
					if (MathLib::isInt(tok2->strAt(2)))
					{
						const MathLib::bigint cur_index(MathLib::toLongNumber(tok2->strAt(2)));
						const Variable *cur_container = symbolDatabase->getVariableFromVarId(cur_varId);
						if (cur_container&&cur_container->dimensions().size()>0 && cur_container->dimension(0)>cur_index)
						{
							bOverRunMaxIndex = false;
						}
					}
					else if ((tok2->tokAt(2)) && (tok2->tokAt(2)->varId() != 0) && tok2->tokAt(2)->getMaxValue(false))
					{
						long long cur_index = tok2->tokAt(2)->getMaxValue(false)->intvalue;
					...
					if ((vecVarTokens.find(cur_varId) != vecVarTokens.end()) && (vecVarCheckedSize.find(cur_varId) == vecVarCheckedSize.end())) //never checked
					{
						if (bOverRunMaxIndex == true)
						{
							stlPossibleOutOfBounds(tok2);
							vecVarCheckedSize.insert(cur_varId);
						}
					}
				}
			}
			if (Token::Match(tok2, "%var% . size ( )") || Token::Match(tok2, "%var% . empty ( )") || Token::Match(tok2, "%var% . resize (") || Token::Match(tok2, "%var% . push_back ("))	//Debug pass empty() check.... usually resize would be  bigger than used. Dont output the mistakes
																																															//	if (Token::Match(tok2, "%var% . size ( )" ))  
			{
				unsigned int cur_sizevarId = tok2->varId();

				if (vecVarTokens.find(cur_sizevarId) != vecVarTokens.end() && (vecVarCheckedSize.find(cur_sizevarId) == vecVarCheckedSize.end()))
				{
					vecVarCheckedSize.insert(cur_sizevarId);
					tok2 = tok2->tokAt(4);
				}
			}
		}
...
// Error  checked 
void CheckStl::stlPossibleOutOfBounds(const Token *tok)
{
	reportError(tok, Severity::warning, ErrorType::BufferOverrun, "stlOutOfBounds", 
		"Possible:: " + tok->str() + " may be out of bounds,you should check " + tok->str() + ".size() before", ErrorLogger::GenWebIdentity(tok->str()));
}


void CheckStl::unexpectedIteratorError(const Token *tok, const std::string &iteratorName)
{
	reportError(tok, Severity::warning, ErrorType::Suspicious, "invalidIterator",
		"Value of iterator [" + iteratorName + "] may be unexpected while doing erase and self-increment operation at the same time.", 
		ErrorLogger::GenWebIdentity(iteratorName));
}
