...
void CheckNonReentrantFunctions::nonReentrantFunctions()
{
    ...
    for (std::size_t i = 0; i < functions; ++i) {
        ...
        for (const Token *tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next()) {
			...
			reportError(tok, Severity::portability, ErrorType::None, "nonreentrantFunctions" + *it, generateErrorMessage(*it), ErrorLogger::GenWebIdentity(tok->str()));
        }
    }
}
//---------------------------------------------------------------------------
...
void CheckNonReentrantFunctions::getErrorMessages(ErrorLogger *errorLogger, const Settings *settings) const
{
    ...
    for (; it!=itend; ++it) {
		c.reportError(0, Severity::portability, ErrorType::None, "nonreentrantFunctions" + *it, generateErrorMessage(*it), ErrorLogger::GenWebIdentity(""));
    }
}
