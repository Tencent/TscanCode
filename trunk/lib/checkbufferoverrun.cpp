...
#ifdef WINGUI
	reportError(tok, Severity::warning,"bufoverrun", "arrayIndexOutOfBounds", oss.str());
#else
    reportError(tok, Severity::error, "bufoverrun","arrayIndexOutOfBounds", oss.str());
#endif
}
...
void CheckBufferOverrun::arrayIndexOutOfBoundsError(const std::list<const Token *> &callstack, const ArrayInfo &arrayInfo, const std::vector<MathLib::bigint> &index)
{
    ...
    makeArrayIndexOutOfBoundsError(oss, arrayInfo, index);
#ifdef WINGUI
	 reportError(callstack, Severity::warning,"bufoverrun", "arrayIndexOutOfBounds", oss.str());
#else
     reportError(callstack, Severity::error,"bufoverrun" ,"arrayIndexOutOfBounds", oss.str());
#endif
}
...
{
#ifdef WINGUI
	 reportError(tok, Severity::warning,"bufoverrun", "bufferAccessOutOfBounds", bufferOverrunMessage(varnames));
#else     
	 reportError(tok, Severity::error,"bufoverrun", "bufferAccessOutOfBounds", bufferOverrunMessage(varnames));
#endif
}
...
{
#ifdef WINGUI
	 reportError(callstack, Severity::warning, "bufoverrun","bufferAccessOutOfBounds", bufferOverrunMessage(varnames));
#else
     reportError(callstack, Severity::error,"bufoverrun", "bufferAccessOutOfBounds", bufferOverrunMessage(varnames));
#endif
}
...
{
    if (cat)
        reportError(tok, Severity::warning,"bufoverrun", "possibleBufferAccessOutOfBounds",
...
void CheckBufferOverrun::possibleReadlinkBufferOverrunError(const Token* tok, const std::string &funcname, const std::string &varname)
{
    ...
    reportError(tok, Severity::warning, "bufoverrun","possibleReadlinkBufferOverrun", errmsg, true);
}
...
void CheckBufferOverrun::strncatUsageError(const Token *tok)
{
		...
		;
#else
    reportError(tok, Severity::warning,"bufoverrun", "strncatUsage",
...
void CheckBufferOverrun::outOfBoundsError(const Token *tok, const std::string &what, const bool show_size_info, const MathLib::bigint &supplied_size, const MathLib::bigint &actual_size)
{
    ...
    oss << '.';
#ifdef WINGUI
    reportError(tok, Severity::warning, "bufoverrun","outOfBounds", oss.str());
#else
	reportError(tok, Severity::error,"bufoverrun", "outOfBounds", oss.str());
#endif
}
...
{
#ifdef WINGUI
	reportError(tok, Severity::warning,"bufoverrun", "pointerOutOfBounds", "Undefined behaviour: Pointer arithmetic result does not point into or just past the end of the " + object + ".\n"
...
#else
	reportError(tok, Severity::portability,"bufoverrun", "pointerOutOfBounds", "Undefined behaviour: Pointer arithmetic result does not point into or just past the end of the " + object + ".\n"
               "Undefined behaviour: The result of this pointer arithmetic does not point into or just one element past the end of the " + object + ". Further information: https://www.securecoding.cert.org/confluence/display/seccode/ARR30-C.+Do+not+form+or+use+out+of+bounds+pointers+or+array+subscripts");
#endif
}
...
{
    reportError(tok, Severity::warning,"bufoverrun", "sizeArgumentAsChar", "The size argument is given as a char constant.");
}
...
{
#ifdef WINGUI
     reportError(tok, Severity::warning,"bufoverrun", "insecureCmdLineArgs", "Buffer overrun possible for long command line arguments.");
#else
	 reportError(tok, Severity::error, "bufoverrun","insecureCmdLineArgs", "Buffer overrun possible for long command line arguments.");
#endif
}
...
void CheckBufferOverrun::bufferNotZeroTerminatedError(const Token *tok, const std::string &varname, const std::string &function)
{
    ...
    reportError(tok, Severity::warning,"bufoverrun", "bufferNotZeroTerminated", errmsg, true);
}
...
void CheckBufferOverrun::parse_for_body(const Token *tok, const ArrayInfo &arrayInfo, const std::string &strindex, bool condition_out_of_bounds, unsigned int counter_varid, const std::string &min_counter_value, const std::string &max_counter_value)
{
    ...
    for (const Token* tok2 = tok; tok2 && tok2 != tok->link(); tok2 = tok2->next()) {
        ...
        if (Token::simpleMatch(tok2, "for (") && Token::simpleMatch(tok2->next()->link(), ") {")) {
            ...
            const Token *startbody = NULL;
			if(endpar)
			startbody=endpar->next();
            const Token *endbody = NULL;
			if(startbody)
			endbody=startbody->link();
            tok2 = endbody;
            continue;
        }
...
void CheckBufferOverrun::checkFunctionParameter(const Token &tok, unsigned int par, const ArrayInfo &arrayInfo, std::list<const Token *> callstack)
{
    ...
    if (it != total_size.end()) {
        ...
        if ((arg == 2 || arg == 3) && tok2) {
            ...
            else if (tok2 && Token::Match(tok2->next(), ",|)") && tok2->type() == Token::eChar) {
            ...
            if (tok2 && Token::Match(tok2, "%num% , %num% ,|)")) {
...
void CheckBufferOverrun::checkFunctionCall(const Token *tok, const ArrayInfo &arrayInfo, std::list<const Token *> callstack)
{
    ...
    // 1st parameter..
    if (tok2 && Token::Match(tok2, "%varid% ,|)", arrayInfo.varid()))
        checkFunctionParameter(*tok, 1, arrayInfo, callstack);
    else if (tok2 && Token::Match(tok2, "%varid% + %num% ,|)", arrayInfo.varid())) {
...
void CheckBufferOverrun::checkScope(const Token *tok, const ArrayInfo &arrayInfo)
{
    ...
    for (const Token* const end = tok->scope()->classEnd; tok != end; tok = tok->next()) {
        ...
        if ((Token::Match(tok, "strncpy|strncat ( %varid% , %var%", arrayInfo.varid()) && Token::Match(tok->linkAt(1)->tokAt(-2), ", %num% )"))) {
            ...
            else if (param3 && tok->str() == "strncat") {
            ...
            if (param3 && Token::Match(param3->tokAt(2), "; strncat ( %varid% ,", arrayInfo.varid()) && Token::Match(param3->linkAt(4)->tokAt(-2), ", %num% )")) {
       ...
       // if (_settings->isEnabled("portability") && Token::Match(tok, "= %varid% + %num% ;", arrayInfo.varid())) {
		 if (Token::Match(tok, "= %varid% + %num% ;", arrayInfo.varid())) {
            const MathLib::bigint index = MathLib::toLongNumber(tok->strAt(3));
...
#ifdef WINGUI
	 reportError(tok, Severity::warning, "bufoverrun","negativeIndex", ostr.str());
#else
    reportError(tok, Severity::error, "bufoverrun","negativeIndex", ostr.str());
#endif
}
...
void CheckBufferOverrun::negativeIndex()
{
    ...
    for (const Token *tok = Token::findmatch(_tokenizer->tokens(), pattern); tok; tok = Token::findmatch(tok->next(),pattern)) {
        ...
        if (index < 0) {
            ...
            if (tok2 && tok2->previous() && tok2->previous()->varId()) {
...
{
#ifdef WINGUI
    reportError(tok, Severity::warning, "bufoverrun","arrayIndexThenCheck",
...
#else
	   reportError(tok, Severity::style,"bufoverrun", "arrayIndexThenCheck",
                "Array index '" + indexName + "' is used before limits check.\n"
                "Defensive programming: The variable '" + indexName + "' is used as an array index before it "
                "is check that is within limits. This can mean that the array might be accessed out of bounds. "
                "Reorder conditions such as '(a[i] && i < 10)' to '(i < 10 && a[i])'. That way the array will "
