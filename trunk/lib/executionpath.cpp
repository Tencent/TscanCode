...
void ExecutionPath::checkScope(const Token *tok, std::list<ExecutionPath *> &checks)
{
    ...
    for (; tok; tok = tok->next()) {
        ...
        if (tok && tok->str() == "break") {
            ExecutionPath::bailOut(checks);
            return;
        }

        if (tok && Token::simpleMatch(tok, "while (")) {
            // parse condition
            ...
            if (Token::simpleMatch(tok, "while ( fgets (")) {
                ...
                if (tok2 != NULL && Token::simpleMatch(tok2, ") ) {")) {
                    tok = tok2->linkAt(2);
                    if (!tok)
                        break;
                    continue;
        ...
        // for/while/switch/do .. bail out
        else if (tok && Token::Match(tok, "for|while|switch|do")) {
            // goto {
            ...
            if (tok->str() == "switch") {
                ...
                for (const Token* tok3 = tok2->next(); tok3; tok3 = tok3->next()) {
                    if (tok3->str() == "{")
                        tok3 = tok3->link();
                    else if (tok3 && tok3->str() == "}")
                        break;
                    else if (tok3 && tok3->str() == "case" &&
            ...
            // no switch
                ...
                if (Token::simpleMatch(tok2->next(), "if (")) {
                    ...
                    if ( tok3 != NULL && Token::simpleMatch(tok3,") {")) {
                        tok3 = tok3->next();
                        ...
                        if ( tok4 != NULL && Token::Match(tok3, "{ %var% =") &&
