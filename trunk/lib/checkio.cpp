...
void CheckIO::checkCoutCerrMisusage()
{
    ...
    for (std::size_t i = 0; i < functions; ++i) {
        ...
        for (const Token *tok = scope->classStart; tok && tok != scope->classEnd; tok = tok->next()) {
            ...
            if (tok && Token::Match(tok, "std :: cout|cerr")) {
...
{
    reportError(tok, Severity::error, "checkIO","coutCerrMisusage", "Invalid usage of output stream: '<< std::" + streamName + "'.");
}
...
{
    reportError(tok, Severity::error,"checkIO",
...
{
    if (portability)
        reportError(tok, Severity::portability,"checkIO",
        ...
        reportError(tok, Severity::warning,"checkIO",
    ...
    for (std::size_t j = 0; j < functions; ++j) {
        ...
        for (const Token *tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next()) {
            ...
            if (Token::Match(tok, "printf|scanf ( %str%")) {
                ...
                } else if (formatStringTok && Token::Match(formatStringTok, "%str% )")) {
                ...
                if (*i == '%') {
                    ...
                    if (scan || *i != 'm') { // %m is a non-standard extension that requires no parameter on print functions.
                        ...
                        if (argListTok && Token::Match(argListTok->next(), "[,)]")) { // We can currently only check the type of arguments matching this simple pattern.
                            ...
                            if (scan && varTypeTok) {
                                if (warning && variableInfo &&((!variableInfo->isPointer() && !variableInfo->isArray()) || varTypeTok->strAt(-1) == "const"))
                                ...
                                switch (*i) {
                                case 's':
                                    if (variableInfo && argListTok->type() != Token::eString && varTypeTok && isKnownType(variableInfo, varTypeTok) && (!variableInfo->isPointer() && !variableInfo->isArray()))
                                        invalidPrintfArgTypeError_s(tok, numFormat);
                                    break;
                                case 'n':
                                    if ((varTypeTok && variableInfo && isKnownType(variableInfo, varTypeTok) && ((!variableInfo->isPointer() && !variableInfo->isArray()) || varTypeTok->strAt(-1) == "const")) || argListTok->type() == Token::eString)
                                ...
                                case 'X':
                                case 'o':
                                    if (varTypeTok && variableInfo && isKnownType(variableInfo, varTypeTok) && !Token::Match(varTypeTok, "bool|short|long|int|char|size_t") && !variableInfo->isPointer() && !variableInfo->isArray())
                                ...
                                case 'd':
                                case 'i':
                                    if (varTypeTok && variableInfo && isKnownType(variableInfo, varTypeTok) && !variableInfo->isPointer() && !variableInfo->isArray()) {
                                ...
                                case 'u':
                                    ...
                                    break;
                                case 'p':
                                    if (varTypeTok && variableInfo && isKnownType(variableInfo, varTypeTok) && !Token::Match(varTypeTok, "short|long|int|size_t") && !variableInfo->isPointer() && !variableInfo->isArray())
                                ...
                                case 'g':
                                case 'G':
                                    if (varTypeTok && variableInfo && ((isKnownType(variableInfo, varTypeTok) && !Token::Match(varTypeTok, "float|double")) || variableInfo->isPointer() || variableInfo->isArray()))
        ...
        unsigned int numFunction)
{
    ...
    reportError(tok, severity,"checkIO", "wrongPrintfScanfArgNum", errmsg.str());
}
...
void CheckIO::invalidScanfArgTypeError(const Token* tok, const std::string &functionName, unsigned int numFormat)
{
    ...
    reportError(tok, Severity::warning,"checkIO", "invalidScanfArgType", errmsg.str());
}
void CheckIO::invalidPrintfArgTypeError_s(const Token* tok, unsigned int numFormat)
{
    ...
    reportError(tok, Severity::warning,"checkIO", "invalidPrintfArgType_s", errmsg.str());
}
void CheckIO::invalidPrintfArgTypeError_n(const Token* tok, unsigned int numFormat)
{
    ...
    reportError(tok, Severity::warning,"checkIO", "invalidPrintfArgType_n", errmsg.str());
}
void CheckIO::invalidPrintfArgTypeError_p(const Token* tok, unsigned int numFormat)
{
    ...
    reportError(tok, Severity::warning,"checkIO", "invalidPrintfArgType_p", errmsg.str());
}
void CheckIO::invalidPrintfArgTypeError_int(const Token* tok, unsigned int numFormat, char c)
{
    ...
    reportError(tok, Severity::warning,"checkIO", "invalidPrintfArgType_int", errmsg.str());
}
void CheckIO::invalidPrintfArgTypeError_uint(const Token* tok, unsigned int numFormat, char c)
{
    ...
    reportError(tok, Severity::warning,"checkIO", "invalidPrintfArgType_uint", errmsg.str());
}
void CheckIO::invalidPrintfArgTypeError_sint(const Token* tok, unsigned int numFormat, char c)
{
    ...
    reportError(tok, Severity::warning, "checkIO","invalidPrintfArgType_sint", errmsg.str());
}
void CheckIO::invalidPrintfArgTypeError_float(const Token* tok, unsigned int numFormat, char c)
{
    ...
    reportError(tok, Severity::warning,"checkIO", "invalidPrintfArgType_float", errmsg.str());
}
void CheckIO::invalidLengthModifierError(const Token* tok, unsigned int numFormat, std::string& modifier)
{
    ...
    reportError(tok, Severity::warning, "checkIO","invalidLengthModifierError", errmsg.str());
}
...
void CheckIO::invalidScanfFormatWidthError(const Token* tok, unsigned int numFormat, int width, const Variable *var)
{
    ...
    if (severity == Severity::error || _settings->isEnabled("style"))
        reportError(tok, severity,"checkIO", "invalidScanfFormatWidth", errmsg.str(), inconclusive);
}
