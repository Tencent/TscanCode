...
{
	reportError(tok, Severity::error, ErrorType::None, "coutCerrMisusage", "Invalid usage of output stream: '<< std::" + streamName + "'.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckIO::fflushOnInputStreamError(const Token *tok, const std::string &varname)
{
    reportError(tok, Severity::portability, ErrorType::None,
		"fflushOnInputStream", "fflush() called on input stream '" + varname + "' may result in undefined behaviour on non-linux systems.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckIO::ioWithoutPositioningError(const Token *tok)
{
    reportError(tok, Severity::error, ErrorType::None,
		"IOWithoutPositioning", "Read and write operations without a call to a positioning function (fseek, fsetpos or rewind) or fflush in between result in undefined behaviour.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckIO::readWriteOnlyFileError(const Token *tok)
{
    ...
    reportError(tok, Severity::error, ErrorType::None,
		"readWriteOnlyFile", "Read operation on a file that was opened only for writing.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckIO::writeReadOnlyFileError(const Token *tok)
{
    reportError(tok, Severity::error, ErrorType::None,
		"writeReadOnlyFile", "Write operation on a file that was opened only for reading.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckIO::useClosedFileError(const Token *tok)
{
    reportError(tok, Severity::error, ErrorType::None,
		"useClosedFile", "Used file that is not opened.", ErrorLogger::GenWebIdentity(tok->str()));
}

void CheckIO::seekOnAppendedFileError(const Token *tok)
{
    reportError(tok, Severity::warning, ErrorType::None,
		"seekOnAppendedFile", "Repositioning operation performed on a file opened in append mode has no effect.", ErrorLogger::GenWebIdentity(tok->str()));
}
        ...
        unsigned int numFunction)
{
	...
	reportError(tok, severity, ErrorType::None, "wrongPrintfScanfArgNum", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
        ...
        unsigned int index, unsigned int numFunction)
{
	...
	reportError(tok, Severity::warning, ErrorType::None, "wrongPrintfScanfParameterPositionError", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckIO::invalidScanfArgTypeError_s(const Token* tok, unsigned int numFormat, const std::string& specifier, const ArgumentInfo* argInfo)
{
    ...
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidScanfArgType_s", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
void CheckIO::invalidScanfArgTypeError_int(const Token* tok, unsigned int numFormat, const std::string& specifier, const ArgumentInfo* argInfo, bool isUnsigned)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidScanfArgType_int", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
void CheckIO::invalidScanfArgTypeError_float(const Token* tok, unsigned int numFormat, const std::string& specifier, const ArgumentInfo* argInfo)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidScanfArgType_float", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckIO::invalidPrintfArgTypeError_s(const Token* tok, unsigned int numFormat, const ArgumentInfo* argInfo)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidPrintfArgType_s", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
void CheckIO::invalidPrintfArgTypeError_n(const Token* tok, unsigned int numFormat, const ArgumentInfo* argInfo)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidPrintfArgType_n", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
void CheckIO::invalidPrintfArgTypeError_p(const Token* tok, unsigned int numFormat, const ArgumentInfo* argInfo)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidPrintfArgType_p", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckIO::invalidPrintfArgTypeError_int(const Token* tok, unsigned int numFormat, const std::string& specifier, const ArgumentInfo* argInfo)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidPrintfArgType_int", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
void CheckIO::invalidPrintfArgTypeError_uint(const Token* tok, unsigned int numFormat, const std::string& specifier, const ArgumentInfo* argInfo)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidPrintfArgType_uint", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckIO::invalidPrintfArgTypeError_sint(const Token* tok, unsigned int numFormat, const std::string& specifier, const ArgumentInfo* argInfo)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidPrintfArgType_sint", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
void CheckIO::invalidPrintfArgTypeError_float(const Token* tok, unsigned int numFormat, const std::string& specifier, const ArgumentInfo* argInfo)
{
    ...
    argumentType(errmsg, argInfo);
    errmsg << ".";
	reportError(tok, Severity::warning, ErrorType::None, "invalidPrintfArgType_float", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckIO::invalidLengthModifierError(const Token* tok, unsigned int numFormat, const std::string& modifier)
{
	...
	reportError(tok, Severity::warning, ErrorType::None, "invalidLengthModifierError", errmsg.str(), ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckIO::invalidScanfFormatWidthError(const Token* tok, unsigned int numFormat, int width, const Variable *var)
{
    ...
    if (arrlen > width) {
        ...
        errmsg << "Width " << width << " given in format string (no. " << numFormat << ") is smaller than destination buffer"
               << " '" << varname << "[" << arrlen << "]'.";
        reportError(tok, Severity::warning, ErrorType::None, "invalidScanfFormatWidth_smaller", errmsg.str(), 0U, true);
    } else {
        ...
        reportError(tok, Severity::error, ErrorType::None, "invalidScanfFormatWidth", errmsg.str(), 0U, false);
    }
}
