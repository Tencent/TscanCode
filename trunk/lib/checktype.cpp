...
bool CheckType::IsExprIntType(const Token* tok)
{
	if (const Variable* pVar = tok->variable())
	{
		return pVar->isIntegralType();
	}
	else if (tok->str() == "(")
	{
		if (iscast(tok))
	...
	{
		const Variable* pVar2 = tok->astOperand2()->variable();
		if (pVar2)
		{
			return pVar2->isIntegralType();
		}
	}
	return false;
}

bool CheckType::IsExprUnsigned(const Token * tok)
{
	if (const Variable* pVar = tok->variable())
	{
		return pVar->isIntegralType() && pVar->typeStartToken()->isUnsigned();
	}
	else if (tok->str() == "(")
	{
		if (iscast(tok))
	...
	else if (tok->str() == "." && tok->astOperand2())
	{
		...
		if (pVar2)
		{
			return pVar2->isIntegralType() && pVar2->typeStartToken()->isUnsigned();
		}
	}
...
void CheckType::IntegerMultiplyOverflowError(const Token *tok)
{
	reportError(tok,
		Severity::warning, ErrorType::Logic,
		"integerMultiplyOverflow",
		"Multiply operation between integer types may cause overflow", tok->astString());
}

void CheckType::UnsignedMinusError(const Token *tok)
{
	reportError(tok,
		Severity::warning, ErrorType::Logic,
		"unsignedMinusError",
		"Minus operation with unsigned variables may cause overflow", tok->astString());
}
...
void CheckType::tooBigBitwiseShiftError(const Token *tok, int lhsbits, const ValueFlow::Value &rhsbits)
{
    ...
    reportError(callstack, rhsbits.condition ? Severity::warning : Severity::error, ErrorType::None, "shiftTooManyBits", errmsg.str(), 0U, rhsbits.inconclusive);
}
...
void CheckType::integerOverflowError(const Token *tok, const ValueFlow::Value &value)
{
    ...
    reportError(tok,
                value.condition ? Severity::warning : Severity::error, ErrorType::None,
                "integerOverflow",
...
void CheckType::signConversionError(const Token *tok)
{
    ...
    reportError(tok,
                ...
                "signConversion",
				"Suspicious code: sign conversion of " + varname + " in calculation, even though " + varname + " can have a negative value", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckType::checkIntegerMultiply()
{
	const SymbolDatabase *symbolDatabase = _tokenizer->getSymbolDatabase();
	const std::size_t functions = symbolDatabase->functionScopes.size();
	for (std::size_t i = 0; i < functions; ++i) 
	{
		...
		for (const Token* tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next())
		{
			if (tok->str() == "*")
			{
				...
				if (tok1 && tok2)
				{
					if (IsExprIntType(tok1) && IsExprIntType(tok2))
					{
						IntegerMultiplyOverflowError(tok);
					}
				}
...
void CheckType::checkUnsignedMinus()
{
	const SymbolDatabase *symbolDatabase = _tokenizer->getSymbolDatabase();
	const std::size_t functions = symbolDatabase->functionScopes.size();
	for (std::size_t i = 0; i < functions; ++i)
	{
		...
		for (const Token* tok = scope->classStart->next(); tok != scope->classEnd; tok = tok->next())
		{
			if (tok->str() == "-")
			{
				...
				if (tok1 && tok2)
				{
					if (IsExprUnsigned(tok1) && !tok2->isNumber())
					{
						UnsignedMinusError(tok);
					}
				}
...
void CheckType::longCastAssignError(const Token *tok)
{
    reportError(tok,
				...
				"int result is assigned to long variable. If the variable is long to avoid loss of information, then there is loss of information. To avoid loss of information you must cast a calculation operand to long, for example 'l = a * b;' => 'l = (long)a * b;'.", ErrorLogger::GenWebIdentity(tok->str()));
}
...
void CheckType::longCastReturnError(const Token *tok)
{
    reportError(tok,
				...
				"int result is returned as long value. If the return value is long to avoid loss of information, then there is loss of information. To avoid loss of information you must cast a calculation operand to long, for example 'return a*b;' => 'return (long)a*b'.", ErrorLogger::GenWebIdentity(tok->str()));
}
