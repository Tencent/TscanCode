...
class TSCANCODELIB CheckObsoleteFunctions : public Check {
public:
...
private:
    ...
    void getErrorMessages(ErrorLogger *errorLogger, const Settings *settings) const {
        ...
        for (std::map<std::string, std::string>::const_iterator it = _obsoleteStandardFunctions.begin(); it != _obsoleteStandardFunctions.end(); ++it)
			c.reportError(0, Severity::style, ErrorType::None, "obsoleteFunctions" + it->first, it->second, ErrorLogger::GenWebIdentity(""));
