...
bool isSameExpression(bool cpp, const Token *tok1, const Token *tok2, const std::set<std::string> &constFunctions)
{
	...
	//eg.if(sizeof(double) == sizeof(long double))
	if (tok1->originalName() != tok2->originalName())
	{
		return false;
	}
	// eg. if ((szBuff != NULL )&&( szBuff != '\0')) not the same
	if (tok1->str() == "0" && tok1->originalName() != tok2->originalName())
		return false;
	...
	//if (!conn.set_option(new mysqlpp::ReadTimeoutOption(20)) ||!conn.set_option(new mysqlpp::WriteTimeoutOption(20))::ReadTimeoutOption
	//ast failed
	if (!tok1->astOperand1() && !tok2->astOperand1() && tok1->tokAt(2) && tok2->tokAt(2) && tok1->tokAt(1)->str() == "::" &&   tok2->tokAt(1)->str() == "::")
	{
		if (tok1->tokAt(2)->str() != tok2->tokAt(2)->str())
			return false;
	}
	//
	bool noncommuative_equals = isSameExpression(cpp, tok1->astOperand1(), tok2->astOperand1(), constFunctions);
...
bool isWithoutSideEffects(bool cpp, const Token* tok)
{
	...
	if (tok && tok->varId()) {
		...
		//todo if var is null return true
		if (!var)
			return true;
		else
