...
public:
	...
	bool checkUninitInForHead(const Token *tok, const Variable& var);
    bool isVariableUsage(const Token *vartok, const Variable& var) const;
	...
	Token *GetFeatureCode(const Token *tok, const Token* tokEnd, const Variable& var);
	FeatureType ParseFeatureCode_switch_body(const Token* tokHead, const Token*& tokUse);
